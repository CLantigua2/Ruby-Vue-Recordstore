<template>
    <div class="max-w-md m-auto py-10">
        ...
        <form @submit.prevent="addArtist">
            <div class="mb-6">
                <input type="text" class="input" autofocus autocomplete="off" placeholder="Type an artist name" v-model="newArtist.name"
                />
            </div>
                ...
                <div v-if="artist == editedArtist">
                    <form @submit.prevent="updateArtist(artist)">
                        <div class="mb-6 p-4 bg-white rounded-border border-grey-light-mt-4">
                            ...
                            <input type="submit" value="Update" class="my-2 bg-transparent text-sm hover:bg-blue hover:text-white text-blue border border-blue no-underline font-bold py-2 px-4 rounded curstor-pointer">
                        </div>
                    </form>
                </div>
            </li>
...
<script>
    export default {
        ...
        removeArtist () {
            ...
            .then(response => {
                this.artist.splice(this.artists.indexOf(artist), 1)
            })
            .catch(error => this.setError(error, "Cannot delete that artist"))
        },
        editArtist (artist) {
